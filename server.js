...
app.post('/create-user',function (req, res){
   //username ,password 
   //{"username": "hemant","password" :  "password1"}
   //JSON
...
app.post('/login', function(req, res) {
    ...
    pool.query('SELECT * FROM "user11" WHERE username = $1', [username],function (err ,result){
        ...
        if (result.rows.length ===0){
            ...
            var hashedPassword = hash(password, salt);//creating hash based on the password submitted and the origional salt
...
app.get('/check-login',function (req ,res){
   if (req.session && req.session.auth && req.session.auth.userId){
       res.send('You Logged in:' + req.session.auth.userId.toString());
   } else {
